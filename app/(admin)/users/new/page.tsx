...
export default function NewUserPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Use Supabase Admin API via edge function, or insert directly if you
      // have service role. Here we insert into profiles; auth user is created
      // on first login via OTP (Supabase's signInWithOtp creates user if
      // they match a pre-inserted profile by email).
      const { error: insertError } = await supabase.from('profiles').insert({
        id: crypto.randomUUID(),   // placeholder; overwritten on first auth
  ...
  if (success) {
    return (
      <div className="mx-auto max-w-sm pt-10 text-center space-y-4">
        ...
        <p className="text-sm text-gray-500">
          <strong>{form.email}</strong> can now sign in. They&apos;ll be prompted to complete their profile on first login.
        </p>
        <div className="flex gap-3 justify-center">
          <button onClick={() => { setSuccess(false); setForm({ email: '', full_name: '', phone: '' }) }}
            className="btn-secondary">Add another</button>
  ...
  return (
    <div className="mx-auto max-w-md space-y-5">
      ...
      <div className="flex items-start gap-2 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-700">
        ...
        The admin will be prompted to complete their profile on first login using a one-time code sent to their email.
      </div>
